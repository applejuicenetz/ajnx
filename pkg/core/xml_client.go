...
func (c *XMLCoreClient) AddShareDir(ctx context.Context, path string, withSubs bool) error {
	mode := "single"
	if withSubs {
		mode = "subdirectory"
	}
	resp, err := c.doRequest(ctx, fmt.Sprintf("/function/addshare?directory=%s&sharesubs=%s", url.QueryEscape(path), mode))
	if err != nil {
		return err
	}
	resp.Body.Close()
...
func (c *XMLCoreClient) RemoveShareDir(ctx context.Context, path string) error {
	resp, err := c.doRequest(ctx, fmt.Sprintf("/function/removeshare?directory=%s", url.QueryEscape(path)))
	if err != nil {
		return err
	}
	resp.Body.Close()
...
func (c *XMLCoreClient) SetShareDirMode(ctx context.Context, path string, withSubs bool) error {
	mode := "single"
	if withSubs {
		mode = "subdirectory"
	}
	resp, err := c.doRequest(ctx, fmt.Sprintf("/function/setsharesubs?directory=%s&sharesubs=%s", url.QueryEscape(path), mode))
	if err != nil {
		return err
	}
	resp.Body.Close()
...
func (c *XMLCoreClient) CheckShares(ctx context.Context) error {
	resp, err := c.doRequest(ctx, "/function/checkshare")
	if err != nil {
		return err
	}
	resp.Body.Close()
...
func (c *XMLCoreClient) GetDirectories(ctx context.Context, path string) ([]string, error) {
	resp, err := c.doRequest(ctx, fmt.Sprintf("/xml/getdirectories.xml?directory=%s", url.QueryEscape(path)))
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
	...
	var dirs []string
	for _, d := range res.Filesystem.Directories {
